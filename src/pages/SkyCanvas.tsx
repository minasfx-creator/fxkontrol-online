...
function TimelineStrip({ time, duration, onSeekAbs }: { time: number; duration: number; onSeekAbs: (t: number) => void }) {
  const ref = useRef<HTMLDivElement | null>(null);
  const onClick = (e: React.MouseEvent<HTMLDivElement>) => {
    const el = ref.current; if (!el) return;
    const rect = el.getBoundingClientRect();
    const t = ((e.clientX - rect.left) / rect.width) * duration;
    onSeekAbs(Math.max(0, Math.min(duration, t)));
  };
  const pct = duration > 0 ? (time / duration) * 100 : 0;
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-8 items-center justify-between px-3 border-b border-cyan-500/10">
        <span className="ds-mono text-[10px] tracking-wider text-cyan-300/80">TIMELINE</span>
        <span className="ds-mono text-[10px] text-zinc-500">FPS 30 · SMPTE 29.97</span>
      </div>
      <div ref={ref} onClick={onClick} className="relative flex-1 cursor-crosshair bg-[#070b14]" role="slider" aria-label="Timeline" aria-valuemin={0} aria-valuemax={Math.round(duration)} aria-valuenow={Math.round(time)}>
        {/* Ruler */}
