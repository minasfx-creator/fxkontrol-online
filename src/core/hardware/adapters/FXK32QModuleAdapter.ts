/**
 * ─── FXK32Q Module Adapter ────────────────────────────────────────
 * 32-channel relay module driven by an ESP32-S3 + 2× 16ch relay banks.
 * Mirror of `FXK16ModuleAdapter` para 32 canais.
 *
 * Speaks the SAME ASCII handshake as FireOneHardwareBridge:
 *   FIRE:<pin>:<ms> | BATCH:<mask32hex>:<ms> | STATUS | VERSION | ESTOP
 * Identification line: "MODEL:FXK32Q;CH:32".
 *
 * Transports suportados (todos vão pelo mesmo handshake VERSION/STATUS):
 *   • USB-CDC (Web Serial)         → 'serial_usb'
 *   • BLE / BLE Long Range         → 'ble'
 *   • WebSocket / Wi-Fi Direct     → 'wifi' / 'ethernet_tcp'
 *   • Art-Net / sACN (UDP)         → 'ethernet_udp'  (promovido via ArtPoll)
 *   • RS-485 XLII+ slave           → 'serial_usb'    (chega via USB↔RS485)
 *
 * STRICTLY READ-ONLY no boundary do registry — todo disparo flui
 * por UI → uiCommandGateway → CommandBus → SafetyStateMachine.
 */
...
export class FXK32QModuleAdapter implements HardwareAdapter<RelayBankState> {
  readonly deviceId = 'fxk32q-esp32s3';
  readonly deviceType = 'relay-bank' as const;
  readonly label = 'FXK32Q — 32ch (ESP32-S3 + 2×16-relay)';
  readonly protocolFamily = 'fireone-ascii' as const;
  readonly firmwareModel = 'FXK32Q' as const;
  readonly compatibleWith = 'fireone_ifmx_i32q' as const;
  ...
  runDiagnostics(): { healthy: boolean; issues: string[] } {
    const issues: string[] = [];
    if (this._connected !== 'connected') issues.push('FXK32Q module not connected');
  ...
  /**
   * Promote to LIVE READ-ONLY após handshake verificado
   * (`MODEL:FXK32Q;CH:32` em qualquer transport: USB/BLE/Wi-Fi/Art-Net/RS-485).
   * Chamado por `discoveryRegistryBridge` — UI nunca chama direto.
   */
...
export const fxk32qModuleAdapter = new FXK32QModuleAdapter();
