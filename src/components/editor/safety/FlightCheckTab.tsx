/**
 * FlightCheckTab — Skybrush-style trajectory safety validation
 * 
 * Extracted from SafetyCheckPanel.tsx for the unified tabbed safety view.
 * Zero behavioral changes — identical logic and rendering.
 */
import { useState, useMemo, useCallback } from 'react';
import { useProjectStore } from '@/store/useProjectStore';
import { Shield, AlertTriangle, CheckCircle, XCircle, Play, Download, Settings, ChevronDown, ChevronRight, Radio } from 'lucide-react';
...
export default function FlightCheckTab() {
  ...
  const hwChecks = useMemo(() => {
    ...
    if (fireone.isConnected) {
      ...
      checks.push({ label: 'FireOne Link', status: 'pass', detail: `${modules.length} modules on RS-485` });
      checks.push({ label: 'FireOne Battery', status: lowBatt.length > 0 ? 'fail' : 'pass', detail: lowBatt.length > 0 ? `${lowBatt.length} modules < 11.0V` : 'All modules OK' });
    ...
    if (pbus.isConnected) {
      ...
      checks.push({ label: 'PBUS Link', status: 'pass', detail: `${devices.length} Showven devices` });
      checks.push({ label: 'PBUS Battery', status: lowBatt.length > 0 ? 'fail' : 'pass', detail: lowBatt.length > 0 ? `${lowBatt.length} devices < 11.0V` : 'All devices OK' });
  ...
  const droneTrajectories = useMemo(() => {
    ...
    for (let i = 0; i < droneCount; i++) {
      ...
      for (const f of droneFormations) {
        ...
        if (point) {
          waypoints.push({ time: f.startTime, position: { x: point.x, y: f.height + point.z, z: 0 } });
          waypoints.push({ time: f.startTime + f.transitionDuration + f.holdDuration, position: { x: point.x, y: f.height + point.z, z: 0 } });
        }
      }
