/**
 * DeconflictionTab — Real-time HCA cross-validation + geofence checks
 * 
 * Extracted from the original SafetyPanel.tsx for the unified tabbed view.
 * Zero behavioral changes — identical logic and rendering.
 */
import { useMemo, useState } from 'react';
import { Shield, AlertTriangle, AlertOctagon, CheckCircle, Settings2, Fence, Zap, Activity, CircleDot, ChevronDown, ChevronRight } from 'lucide-react';
    ...
    <div className="space-y-2">
      ...
      <div className="bg-surface-2 rounded-sm overflow-hidden">
        ...
        >
          ...
          <Switch checked={showHCA} onCheckedChange={setShowHCA} className="h-4 w-7" onClick={e => e.stopPropagation()} />
        </div>
      ...
      <div className="bg-surface-2 rounded-sm overflow-hidden">
        <div className="flex items-center justify-between p-2">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => setShowGeofenceSettings(!showGeofenceSettings)}>
              <Settings2 className="h-3 w-3" />
            </Button>
            <Switch checked={geofence.enabled} onCheckedChange={(v) => setGeofence({ ...geofence, enabled: v })} className="h-4 w-7" />
          </div>
        </div>
