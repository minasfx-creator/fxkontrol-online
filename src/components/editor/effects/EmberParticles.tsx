 ...
 * Adds realism with individual flicker, wind drift, and gravity.
 */
export default function EmberParticles({
  ...
  const seeds = useMemo(() => {
    ...
    for (let i = 0; i < EMBER_COUNT; i++) {
      ...
      s.push({
        ...
        lt: 1.5 + Math.random() * 3,
        flicker: 15 + Math.random() * 40,
      });
    }
    return s;
  }, [spreadRadius]);
  ...
  useFrame(({ clock }) => {
    if (!pointsRef.current || progress < 0.2) return;
    ...
    for (let i = 0; i < EMBER_COUNT; i++) {
      ...
      posArr[i * 3 + 1] = y;
      posArr[i * 3 + 2] = seed.z + Math.sin(time * 0.5 + i) * 0.2;
      ...
      // Dim ember color: orange → red → dark
      const dimFactor = Math.pow(fade, 0.5);
      colArr[i * 3] = baseColor.r * dimFactor * flicker * 0.8;
      colArr[i * 3 + 1] = baseColor.g * dimFactor * flicker * 0.4;
      colArr[i * 3 + 2] = baseColor.b * dimFactor * flicker * 0.2;
    }
