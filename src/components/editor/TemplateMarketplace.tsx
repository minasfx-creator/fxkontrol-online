/**
 * Template Marketplace — Browse & share show templates via cloud storage
 */
...
// Sample cloud templates (simulated)
...
export default function TemplateMarketplace({ onClose }: TemplateMarketplaceProps) {
  ...
  const handleDownload = (template: CloudTemplate) => {
    toast.success(`Template "${template.name}" downloaded! Open Templates panel to load it.`);
  };

  const handlePublish = () => {
    if (localTemplates.length === 0) {
      toast.error('No local templates to publish. Save a template first in the Templates panel.');
      return;
    }
        ...
        <div className="p-3 space-y-3">
            ...
            <>
              ...
              <div className="flex flex-wrap gap-1">
                <Badge
                  ...
                  className="text-[8px] cursor-pointer h-4"
                ...
                {filtered.map(t => (
                  <div key={t.id} className="bg-surface-2/60 rounded-md p-2.5 border border-border/30 space-y-1.5 hover:border-primary/30 transition-colors">
                    <div className="flex items-start justify-between">
                      <div>
                        <h4 className="text-[11px] font-semibold text-foreground">{t.name}</h4>
                    ...
                    <div className="flex items-center gap-3 text-[8px] text-muted-foreground">
                      ...
                      <span className="flex items-center gap-0.5">
                        ...
                        {t.formationCount} formations
                      </span>
                    </div>

                    <div className="flex gap-1">
                      {t.tags.map(tag => (
                        <span key={tag} className="text-[7px] bg-surface-3/60 px-1.5 py-0.5 rounded text-muted-foreground">
                          #{tag}
                        </span>
                      ))}
                    </div>
            ...
            <div className="space-y-2">
              {localTemplates.length === 0 ? (
                <p className="text-[10px] text-muted-foreground text-center py-6">
                  No saved templates yet. Use the Templates panel to save one.
                </p>
              ) : (
                localTemplates.map(t => (
                  <div key={t.id} className="bg-surface-2/60 rounded p-2 border border-border/30">
                    <h4 className="text-[10px] font-semibold text-foreground">{t.name}</h4>
                    <p className="text-[9px] text-muted-foreground">{t.description}</p>
                    <Button size="sm" variant="outline" className="w-full h-5 text-[9px] mt-1.5" onClick={handlePublish}>
                      <Upload className="w-3 h-3 mr-1" /> Publish to Marketplace
                    </Button>
                  </div>
                ))
              )}
          ...
          {tab === 'publish' && (
            <div className="space-y-3 text-center py-4">
              ...
              <h4 className="text-[11px] font-semibold text-foreground">Share Your Templates</h4>
              <p className="text-[9px] text-muted-foreground leading-relaxed">
                Publish your show templates to the marketplace for other designers to use.
                Templates include formations, scene settings, and metadata.
              </p>
