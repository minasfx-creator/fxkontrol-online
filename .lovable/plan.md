...
- ⏳ Próximos: (a) loop `play` em workMode=`simulation` rodando o golden seed na Show3DEngine (validar ParticleGPGPU + Smoke + Bloom de ponta a ponta); (b) critério de saída Fase 1 — `readinessEvaluator.evaluate().status === 'READY_FOR_EXPORT'`.
