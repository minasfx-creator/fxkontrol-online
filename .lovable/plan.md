...
- ⏳ Próximos: (a) acoplar dry-run ao `Show3DEngine` real para validação visual end-to-end de ParticleGPGPU + Smoke + Bloom tocando o golden seed; (b) **Fase 1 fechada no software** — pode iniciar Fase 2 (hardware físico em mesa) em paralelo, ou expandir biblioteca de golden seeds (Maracanã noturno, fechamento de campeonato, abertura de show de drone).
