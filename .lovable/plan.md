...
- ⏳ Próximos: (a) loop `play` em workMode=`simulation` validando shaders cinema (ParticleGPGPU + Smoke + Bloom); (b) critério de saída Fase 1 — `readinessEvaluator.evaluate().status === 'READY_FOR_EXPORT'`.
