...
- ⏳ Próximos: (a) export FireOne + export Skybrush honest do Libertadores (claim policy + `_FXK_DISCLAIMER.txt`); (b) loop `play` em workMode=`simulation` validando shaders cinema; (c) critério de saída Fase 1 — `READY_FOR_EXPORT`.
